...
const posts = (await Promise.all((await getCollection("posts") as Post[]).map(async post => {
  ...
  return post
}))).sort((postA,postB)=>postA.data.publishDate!.valueOf() - postB.data.publishDate!.valueOf())
